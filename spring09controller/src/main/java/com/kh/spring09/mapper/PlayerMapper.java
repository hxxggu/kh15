...
public class PlayerMapper implements RowMapper<PlayerDto> {
	...
	public PlayerDto mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		playerDto.setPlayerNo(rs.getInt("player_gold_medal"));
		playerDto.setPlayerNo(rs.getInt("player_silver_medal"));
		playerDto.setPlayerNo(rs.getInt("player_bronze_medal"));
		return playerDto;
	}
